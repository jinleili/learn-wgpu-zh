...
如果需要支持模拟器运行，还得加上 `x86_64-linux-android` 和 `i686-linux-android` 这两个构建目标的支持。需要注意的是，如果在 `InstanceDescriptor` 中显式指定只使用 Vulkan 图形后端（`backends: wgpu::Backends::VULKAN`），则在模拟器内运行时会崩溃：
