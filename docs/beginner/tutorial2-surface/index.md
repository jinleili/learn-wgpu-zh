...
- `apply_limit_buckets` 会把适配器的能力和限制调整到预定义档位。这里保持 `false`，直接使用适配器报告的能力。
...
`color_space` 定义展示平面的色彩空间。`Auto` 会保持 wgpu 以往的默认行为，适合不需要主动选择广色域或 HDR 输出的应用。
...
在 wgpu v30 中，`Surface::get_current_texture()` 返回的是 `CurrentSurfaceTexture` 枚举。为了保持示例代码简洁，这里使用 `utils::get_current_surface_texture()` 帮我们把它转换成 `Result<SurfaceTexture, SurfaceStatus>`。我们将它存储在 `output` 变量中以便后续使用。
